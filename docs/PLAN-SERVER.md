...
describe('slugs reservados', () => {
  it('reserva as rotas estáticas do front', () => {
    expect(RESERVED_SLUGS).toContain('not-found')
  })

  it('reserva os prefixos da própria API', () => {
    expect(RESERVED_SLUGS).toContain('api')
    expect(RESERVED_SLUGS).toContain('docs')
  })

  it('identifica um slug reservado', () => {
    expect(isReservedSlug('not-found')).toBe(true)
  })
...
```ts
/**
 * Slugs que colidiriam com rotas da aplicação.
 *
 * A rota `/:slug` do front disputa a raiz do domínio com as rotas estáticas
 * dele. O roteador prioriza a rota estática, então um link com slug
 * `not-found` seria criado com sucesso e ficaria inalcançável para sempre,
 * sem erro em lugar nenhum.
 *
 * ⚠️ Ao adicionar uma rota estática no front, adicione o slug aqui.
 */
...
describe('POST /links', () => {
  ...
  it('responde 409 para slug reservado', async () => {
    const response = await app.inject({
      method: 'POST',
      url: '/links',
      payload: { originalUrl: 'https://a.com', slug: 'not-found' },
    })
